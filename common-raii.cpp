  ...
  if (pam_get_item(pamh, PAM_CONV, &vconv) == PAM_SUCCESS) {
    ...
    try {
      if (vconv != nullptr && conv != nullptr && conv->conv != nullptr) {
        ...
        if (conv->conv(marr.size(), marr.data(), &rr, conv->appdata_ptr) !=
            PAM_SUCCESS)
          throw runtime_error("App callback failed"s);

        if (rr != nullptr && rr->resp != nullptr) {
          unique_ptr<char[], void (*)(void *)> uniqResp(
              rr->resp, free); // freed by C free()
...
gpgme_key_t &keyRaii::get() { return key; }

string getNonce(int len = 10) {
  static string chars{
      "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"s};
  auto ret{""s};
  random_device rd{};
  mt19937 g{rd()};
...
gpgme_data_raii::gpgme_data_raii(const string &str) {
  if (auto err{gpgme_data_new_from_mem(&data, str.c_str(), str.length(), 1)};
      err != GPG_ERR_NO_ERROR)
    throw runtime_error("Can't init gpgme data from mem "s +
                        string{gpgme_strerror(err)});
}
gpgme_data_raii::gpgme_data_raii() {
  if (auto err{gpgme_data_new(&data)}; err != GPG_ERR_NO_ERROR)
    throw runtime_error("Can't init gpgme empty data "s +
                        string{gpgme_strerror(err)});
}

gpgme_data_t &gpgme_data_raii::get() { return data; }
...
string encrypter::ciphertext(string recp, bool trust, bool sign,
                             string sender) {
...
string encrypter::encPub(string recp, bool trust, bool sign, string sender) {
  ...
  gpgme_encrypt_flags_t params{trust ? GPGME_ENCRYPT_ALWAYS_TRUST
                                     : static_cast<gpgme_encrypt_flags_t>(0)};
  if (sign) {
    if (auto err{gpgme_op_keylist_start(ctx.get(), sender.c_str(), 0)};
        err != GPG_ERR_NO_ERROR)
      throw runtime_error("gpgme_op_keylist_start() failed"s +
                          string{gpgme_strerror(err)});
    keyRaii key;
    if (auto err{gpgme_op_keylist_next(ctx.get(), &key.get())};
        err != GPG_ERR_NO_ERROR)
      throw runtime_error("gpgme_op_keylist_next() failed "s +
                          string{gpgme_strerror(err)});
    if (auto err{gpgme_op_keylist_end(ctx.get())}; err != GPG_ERR_NO_ERROR)
      throw runtime_error("gpgme_op_keylist_end() failed "s +
                          string{gpgme_strerror(err)});
    if (auto err{gpgme_signers_add(ctx.get(), key.get())};
        err != GPG_ERR_NO_ERROR)
      throw runtime_error("Can't add signer "s + sender + " " +
                          string{gpgme_strerror(err)});
    if (auto err{gpgme_op_encrypt_sign_ext(ctx.get(), NULL,
                                           recpFormatted.c_str(), params,
                                           in.get(), out.get())};
        err != GPG_ERR_NO_ERROR) {
      throw runtime_error("Can't encrypt/sign with keys "s + recp + ", " +
                          sender + " : " + string{gpgme_strerror(err)});
    }
  } else {
    if (auto err{gpgme_op_encrypt_ext(ctx.get(), NULL, recpFormatted.c_str(),
                                      params, in.get(), out.get())};
        err != GPG_ERR_NO_ERROR)
      throw runtime_error("Can't encrypt to "s + recp + " "s +
                          string{gpgme_strerror(err)});
  }
...
privDropper::privDropper(pam_handle_t *pam, struct passwd *p)
    : pamh{pam}, origUid{geteuid()}, origGid{getegid()}, dropped{false} {
  if (origUid == 0) {
    if (setegid(p->pw_gid) != 0) // setfsgid() isn't portable
      throw runtime_error{"setegid() failed"s};
    if (seteuid(p->pw_uid) != 0) {
      setegid(
          origGid); // Should be RAII but it's probably useless if we got here
  ...
  if (dropped) {
    if (seteuid(origUid) != 0 || setegid(origGid) != 0) {
      pam_syslog(pamh, LOG_WARNING,
                 "failed regaining privs, remaining pam modules in the stack "
                 "might misbehave");
    }
  }
}
